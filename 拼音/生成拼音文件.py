...
for i in range(len(lines)):
    ...
    if len(line) == 2 and line[0].startswith("U+"):
        ...
        # 检查最后一个字符是否是数字
        last_char = line[1][-1]
        if not last_char.isdigit() and 'a' <= last_char <= 'z':
            line[1] = line[1][:-1] + "q" * 5  # 如果不是数字，则添加5个“q”
        else:
            # 移除音调符号后将最后一个字符替换为相应数量的“q”
            last_char_without_tone = last_char.rstrip("̄́̌̀")
            if last_char_without_tone:
                line[1] = line[1][:-1] + "q" * int(last_char_without_tone)
            else:
                line[1] = line[1][:-1] + "q" * 5  # 如果为空，则添加5个“q”
...
# 打开 temp8_Unihan_Readings.txt 文件
with open("temp8.txt", "r", encoding="utf-8") as f:
    # 读取原文件内容
    content = f.read()
...
# 写入新文件
with open("pinyin-26000.txt", "w", encoding="utf-8") as f:
    f.write(new_content)
